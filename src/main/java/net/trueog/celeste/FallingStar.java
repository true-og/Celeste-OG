...
public class FallingStar extends BukkitRunnable {
    ...
    public void run() {
        ...
        if (y <= dropLoc.getY()) {
            if (!lootDropped) {
                ...
                if (config.fallingStarLootTable != null) {
                    ...
                    String command = String.format(
                            "execute at %s run loot spawn %s %s %s loot %s",
                            marker.getUniqueId(),
                            dropLoc.getX(),
                            dropLoc.getY(),
                            dropLoc.getZ(),
                    ...
                    if (celeste.getConfig().getBoolean("debug")) {
                        celeste.getLogger()
                                .info("Spawned falling star loot from loot table '" + config.fallingStarLootTable
                                        + "'");
                    }
                }
                if (config.fallingStarsExperience > 0) {
                    ExperienceOrb orb =
                            (ExperienceOrb) dropLoc.getWorld().spawnEntity(dropLoc, EntityType.EXPERIENCE_ORB);
                    orb.setExperience(config.fallingStarsExperience);
                    if (celeste.getConfig().getBoolean("debug")) {
                        celeste.getLogger()
                                .info("Dropping experience orbs with value " + config.fallingStarsExperience);
                    }
                }
                lootDropped = true;
            }
            if (y % (step * 5) == 0) {
                location.getWorld()
                        .spawnParticle(Particle.LAVA, dropLoc, 0, 0, new Random().nextDouble(), 0, 1, null, true);
            }
            sparkTimer--;
            if (sparkTimer <= 0) {
                this.cancel();
