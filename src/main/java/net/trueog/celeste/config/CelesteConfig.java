...
public class CelesteConfig {
    ...
    private void buildFromConfigurationSectionWithGlobal(ConfigurationSection section, CelesteConfig globalConfig) {
        ...
        shootingStarsPerMinuteMeteorShower = section.getDouble("shooting-stars-per-minute-during-meteor-showers",
                globalConfig.shootingStarsPerMinuteMeteorShower);
        ...
        adaptiveGlobalPlayerCount = section.getBoolean("adaptive-use-global-player-count",
                globalConfig.adaptiveGlobalPlayerCount);
        ...
        if (section.isSet("falling-stars-loot") || section.isSet("falling-stars-loot-table")) {
            ...
            // Ensure that neither type of loot is inherited from the global config if any
            // overrides are used
            // This means that even if the global config sets simple loot and the world
            // config overrides just loot
            // table, the end result will be a config with the loot table set and the simple
            // table empty
            fallingStarLootTable = section.getString("falling-stars-loot-table");
            if (section.isSet("falling-stars-loot")) {
